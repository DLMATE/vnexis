...
from vnexis.utils import draw_text_using_idx, resize_boxes
...
class WebDisplayer(AsyncEventHandler[KeyFrameDetectionDone]):
    ...
    def process(self, event: KeyFrameDetectionDone):
        ...
        frame = self._render_frame(client_id, event)
    ...
    def _render_frame(self, client_id: int, event: KeyFrameDetectionDone) -> np.ndarray:
        ...
        data = draw_text_using_idx(
            data,
            f"detect_time: {event.metadata.key_frame_detect_time:.3f}",
            3,
            (0, 255, 0),
        )
        return data
